...
class Utilities:
    ...
    def update_config_with_default(configDict, defaultDict):
        ...
        for key, default_value in defaultDict.items():
            if key not in configDict:
                ...
                configDict[key] = Utilities.update_config_with_default(
                    configDict.get(key, {}), default_value
                )
        return configDict
    ...
    def aggregate_group_metrics(
        models: list[Union[OrderedDict, torch.nn.Module]],
        metric_fn: callable,
        aggregate_fn: callable,
    ) -> float:
        if len(models) == 1:
            sys.stdout.write(
                "Warning: aggregate_group_metrics called with only one model. Returning 0.\n"
            )
            return 0.0
    ...
    @torch.no_grad()
    def get_angle(
        model_a: Union[OrderedDict, torch.nn.Module],
        model_b: Union[OrderedDict, torch.nn.Module],
    ) -> float:
        ...
        dot_product = 0.0
        squared_norm_a, squared_norm_b = 0.0, 0.0
    ...
    @staticmethod
    @torch.no_grad()
    def get_barycentre_l2_distance(
        models: list[Union[OrderedDict, torch.nn.Module]], maximize=True
    ):
    ...
    def permutation_spec_from_axes_to_perm(axes_to_perm: dict) -> PermutationSpec:
        ...
        return PermutationSpec(
            perm_to_axes=dict(perm_to_axes), axes_to_perm=axes_to_perm
        )
    ...
    def get_overloaded_dataset(OriginalDataset):
        class AlteredDatasetWrapper(OriginalDataset):
            ...
            def __getitem__(self, index):
                ...
                image, label = super(AlteredDatasetWrapper, self).__getitem__(
                    index=index
                )
...
class CalibrationError(torchmetrics.Metric):
    def __init__(self, num_bins=15, norm="l1", dist_sync_on_step=False):
        ...
        self.add_state(
            "bin_boundaries",
            default=torch.linspace(0, 1, num_bins + 1),
            dist_reduce_fx=None,
        )
        self.add_state(
            "bin_conf_sums", default=torch.zeros(num_bins), dist_reduce_fx="sum"
        )
        self.add_state(
            "bin_correct_sums", default=torch.zeros(num_bins), dist_reduce_fx="sum"
        )
        self.add_state(
            "bin_total_count", default=torch.zeros(num_bins), dist_reduce_fx="sum"
        )
...
class Candidate(object):
    ...
    def __init__(self, candidate_id, candidate_file, candidate_run):
        ...
        self._candidate_metrics = defaultdict(
            defaultdict
        )  # 'test'/'val'/'ood' -> {metric -> value}
