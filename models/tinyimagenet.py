...
def _resnet_tiny(arch, block, layers, pretrained, progress, **kwargs):

    model = ResNet(block, layers, num_classes=200, **kwargs)

    return model

def resnet18_tiny(args, **kwargs):
    """FP32 ResNet-18 model adapted for Tiny ImageNet."""

    pretrained = getattr(args, 'pretrained', False)
    return _resnet_tiny('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress=True, **kwargs)

def resnet34_tiny(args, **kwargs):
    """FP32 ResNet-34 model adapted for Tiny ImageNet."""
    pretrained = getattr(args, 'pretrained', False)
    return _resnet_tiny('resnet34', BasicBlock, [3, 4, 6, 3], pretrained, progress=True, **kwargs)
...
__all__ = ['resnet18_quant_tiny', 'resnet34_quant_tiny']
...
class QResNet(nn.Module):
    ...
    def __init__(self, args, block, layers, num_classes=200, zero_init_residual=False, 
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
...
def _resnet_quant_tiny(args, arch, block, layers, pretrained, progress, **kwargs):
    model = QResNet(args, block, layers, num_classes=200, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch], progress=progress)
        print("Load pretrained full-precision ImageNet weights")
        model.load_state_dict(state_dict, strict=False)
    return model

def resnet18_quant_tiny(args, pretrained=False, progress=True, **kwargs):
    """ResNet-18 model adapted for Tiny ImageNet (64x64 images, 200 classes).
    
    Args:
        pretrained (bool): If True, returns a model with weights pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_quant_tiny(args, 'resnet18', QBasicBlock, [2, 2, 2, 2], pretrained, progress,
                       **kwargs)

def resnet34_quant_tiny(args, pretrained=False, progress=True, **kwargs):
    """ResNet-34 model adapted for Tiny ImageNet (64x64 images, 200 classes).

    Args:
        pretrained (bool): If True, returns a model with weights pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_quant_tiny(args, 'resnet34', QBasicBlock, [3, 4, 6, 3], pretrained, progress,
                       **kwargs)
